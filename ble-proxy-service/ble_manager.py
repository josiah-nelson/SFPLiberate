"""
BLE Manager for standalone proxy service.

Simplified version of the backend BLE manager focused on core operations
for the SFP Wizard device communication.
"""
...
class BLEManager:
    ...
    async def discover_devices(
        ...
        Args:
            ...
            adapter: Optional adapter (e.g., 'hci0')
    ...
    async def connect(
        ...
        Args:
            service_uuid: Optional service UUID for auto-discovery
            device_address: Optional specific device address
            adapter: Optional adapter (e.g., 'hci0')
        ...
        if not device_address:
            if not service_uuid:
                raise ValueError("Either device_address or service_uuid is required")
        ...
        kwargs = {"address_or_ble_device": device_address, "disconnected_callback": self._on_disconnect}
        if adapter:
            kwargs["adapter"] = adapter
        
        self.client = BleakClient(**kwargs)
        ...
        try:
            ...
            if hasattr(self.client, "_device_info"):
                device_name = self.client._device_info.get("name", "Unknown")
